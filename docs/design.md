...
- **保存時暗号化（任意）**：`PK_DB_PASSPHRASE` を設定すると SQLite ファイル全体（FTS5・WAL 含む）を
  SQLCipher で暗号化。復号はメモリ上のため検索・利便性は不変。スナップショットは `VACUUM INTO` で
  暗号化を継承し、平文⇄暗号の移行は `PRAGMA rekey`（`npm run db:encrypt`/`db:decrypt`）。マイナンバー等を
  入れる場合の最低限の備え（合言葉を失うと復元不能なので別途厳重保管）。
...
- **ログマスキング**：出力は `src/redact.ts` を単一チョークポイントとして再帰的にマスクし、
  マイナンバー・パスワード等の秘匿キー・Bearer トークンが平文で残らないようにする（多層防御。
  そもそも本文や鍵を detail に渡さないのが原則だが、将来の事故も一箇所で抑える）
