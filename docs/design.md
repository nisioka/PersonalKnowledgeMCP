...
```
[収集層 Ingestion]
  ├─ Discord bot（ファイル投入：画像/PDF → OCR → 構造抽出）
  └─ MCP register ツール（テキスト投入：Claudeから登録）
        │
        ▼
...
| コンポーネント | 言語 | 理由 |
|---|---|---|
| MCPサーバ本体 | TypeScript / Node.js | 本業主力で開発・保守が最速。静的型でスキーマ駆動に向き「後から育てる」設計と相性良。`@modelcontextprotocol/sdk` が充実 |
| Discord bot | TypeScript | discord.js が定番・高品質。MCPサーバと同居しやすい |
| OCR・構造抽出 | Python | PaddleOCR・PDF処理・データ整形ライブラリが強い。OCRはPythonが明確に有利 |

### 連携方式
TS（常駐部分）から Python（OCR/抽出の重い処理）を子プロセス or 内部HTTPで呼ぶ。両言語とも既存資産（家計分割システム＝TS、Gmail digest/Whisper系＝Python）と地続き。
...
### Phase 2：収集経路
5. Discord bot（ファイル受信）
6. PaddleOCR による OCR ＋ Claude API での構造抽出（Python、子プロセス/内部HTTP）
7. 抽出プロンプトの外部ファイル化（doc_type別）
8. update ツール（手動上書き・訂正用）＋ 重複の緩い名寄せ
...
| MCPサーバ | TypeScript（@modelcontextprotocol/sdk、Streamable HTTP） |
| Discord bot | TypeScript（discord.js） |
| OCR・構造抽出 | Python（PaddleOCR + Anthropic API） |
...
| 言語 | ハイブリッド（TS＋Python） | 単一言語 → OCRはPython有利、保守はTS有利で両取りが効率的 |
