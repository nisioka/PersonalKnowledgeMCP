...
> **改訂メモ（実装時の変更）**
> 本設計書は当初構想の記録です。実装では、書類取り込みの「OCR＋構造抽出」を
> **自前 Discord bot ＋ PaddleOCR ＋ Anthropic API**で行う案を取りやめ、**Claude 本体に
> 行わせる方式**へ変更しました（理由：Anthropic API は月額 Claude プランと別の従量課金で、
> 家庭利用ではコストが読みにくいため）。
> - 取り込みは、Claude（Code / デスクトップ / アプリ、または Claude Code の Discord 連携
>   「Channels」）に書類を添付し、MCP プロンプト `ingest_document` で Claude 自身が読み取り
>   → `register` を呼ぶ。OCR も Claude のマルチモーダルで賄うため PaddleOCR 不要、API キー不要。
> - これに伴い `src/ingest/`（自前 Discord bot）・`python/`（PaddleOCR/Anthropic サービス）・
>   `prompts/`（抽出プロンプト）は削除（Git 履歴に残存）。
> - 本文中の「Discord bot で OCR」「PaddleOCR」「Python で構造抽出」「能動提案を Discord へ」
>   等の記述は、上記の方針へ読み替えてください（リマインダー通知は Discord webhook を継続使用）。
> 詳細な現行構成は [`../README.md`](../README.md) を参照。
...
```
（当初案：Discord bot がファイルを受け OCR→構造抽出していた。改訂メモ参照）
...
| 項目 | 決定 | 理由 |
|---|---|---|
| 稼働マシン | Ubuntu Server（常時起動） | MCPサーバ・Discord botは常駐プロセス。メインWindows機は再起動/スリープが多く常駐に不向き。24時間到達可能であることが外部アクセスの価値の源泉 |
| メインWindows機 | 非依存（クライアントとしてのみ利用） | メイン用途を圧迫しない |

### リソース見積もり
- OCR（PaddleOCR）は**散発的なバースト負荷**であり持続負荷ではない。書類投入時のみ数秒〜十数秒の処理。リアルタイム性は不要
- MCPサーバ本体・SQLite・ベクトル検索はごく軽量
- ミドルスペックなら余裕、ロースペックでも実用範囲。精度最優先かつ負荷ゼロにしたい場合はOCRのみGoogle Cloud Vision APIへ逃がす選択肢を残す
...
| コンポーネント | 言語 | 理由 |
|---|---|---|
| MCPサーバ本体・各CLI | TypeScript / Node.js | 本業主力で開発・保守が最速。静的型でスキーマ駆動に向き「後から育てる」設計と相性良。`@modelcontextprotocol/sdk` が充実 |

> 当初は「OCR・構造抽出 = Python（PaddleOCR）」「Discord bot = TypeScript（discord.js）」の
> ハイブリッド構成だったが、改訂メモのとおり取り込みを Claude 主導に変更したため Python と
> discord.js は不要となり、実装は TypeScript 単一言語になった。OCR・構造抽出は Claude が担当する。
...
- 抽出ロジックは Claude へのプロンプト（抽出指示テンプレート）として**外部ファイル化**し、doc_type ごとに用意する
- 「保証期限も抽出したい」と思ったら、抽出プロンプトに1行足して再処理するだけ。既存データを壊さない
...
### Phase 2：収集経路 ＋ 訂正
5. ~~Discord bot（ファイル受信）~~ → **Claude 主導の取り込み**（添付 → MCP プロンプト
   `ingest_document` → Claude が OCR/抽出 → `register`）に変更（改訂メモ参照）
6. ~~PaddleOCR + Claude API での構造抽出（Python）~~ → **Claude 本体が担当**（API キー不要）
7. 抽出指示は MCP プロンプト `ingest_document` に集約（doc_type 語彙・期限推定・dedup_key の指針）
8. update / delete（soft・hard）/ restore ツール（手動上書き・訂正・復元）＋ 重複の緩い名寄せ（dedup_key）
9. SQLite の Google Drive バックアップ（暗号化・日次cron）
...
| 書類の OCR・構造抽出 | Claude 本体（MCP プロンプト `ingest_document`、API キー不要）※当初は Python＋PaddleOCR＋Anthropic API |
| 外部公開 | Cloudflare Tunnel |
| 認証・認可 | Cloudflare Access（scopeマッピング） |
| リマインダー通知 | Discord webhook（`discord.js` 不要） |
...
| 言語 | TypeScript 単一（実装時に確定） | 当初はハイブリッド（TS＋Python）を想定したが、取り込みを Claude 主導にしたため Python が不要に |
...
| OCR | PaddleOCR | tesseract（精度/レイアウト弱）、Vision API（負荷ゼロだがネット必須・従量課金）。将来Vision APIへ切替可能に抽象化 |
